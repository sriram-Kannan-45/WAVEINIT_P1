...
const interviewController = {
  ...
  async list(req, res) {
    try {
      const { status, interviewType, page = 1, limit = 20 } = req.query;
      const filters = {};
  ...
  // GET /api/interview/dashboard — Dashboard stats
  ...
  // GET /api/interview/:id — Get interview detail
  ...
  async update(req, res) {
    try {
      ...
      res.status(500).json({ message: error.message || 'Failed to update interview' });
    }
  },
  ...
  // PUT /api/interview/:id/end — End interview
  async end(req, res) {
    try {
  ...
  // POST /api/interview/:id/join — Join interview room
  async join(req, res) {
    try {
  ...
  // POST /api/interview/:id/leave — Leave interview room
  async leave(req, res) {
    try {
      const isCandidate = req.user.role === 'PARTICIPANT';
      if (isCandidate) {
      ...
      await interviewService.logActivity(req.params.id, req.user.id, isCandidate ? 'CANDIDATE_LEFT' : 'CANDIDATE_LEFT', {});
  ...
  // POST /api/interview/:id/participants — Assign participants
  ...
  // POST /api/interview/:id/trainers — Assign trainers
  ...
  // GET /api/interview/participant/my — Get participant's interviews
  ...
  // GET /api/interview/trainer/my — Get trainer's interviews
  async trainerInterviews(req, res) {
    try {
      ...
      console.error('Trainer interviews error:', error);
      res.status(500).json({ message: 'Failed to get interviews' });
    }
  },

  // GET /api/interview/:id/activity — Get activity log
  ...
  // GET /api/interview/:id/notifications — Get notifications
  ...
  // POST /api/interview/:id/verify-password — Verify meeting password
  ...
  // POST /api/interview/:id/qr/generate — Generate QR token
  ...
  // POST /api/interview/:id/qr/verify — Verify QR token
  ...
  // POST /api/interview/:id/mobile/connect — Connect mobile device
  async connectMobile(req, res) {
    try {
      ...
      console.error('Connect mobile error:', error);
      res.status(500).json({ message: 'Failed to connect mobile device' });
    }
  },

  // POST /api/interview/:id/evaluation — Submit evaluation
  ...
  // GET /api/interview/:id/evaluations — Get all evaluations
  ...
  // POST /api/interview/:id/publish-results — Publish results
